...
public:
    Jacobi(){};
    ~Jacobi(){};
    Jacobi(const std::vector<std::vector<double>>& rhs, double eps, int maxN);
    void calEigen();
    void max_E(int& row, int& col, double& maxE);
    void Givens(int p, int q);
    void output();
};

//构造函数
Jacobi::Jacobi(const std::vector<std::vector<double>>& rhs, double eps, int maxN){
    this->a = rhs;
    this->Eps = eps;
    this->MaxNum = maxN;
    this->nDim = rhs.size();
...
void Jacobi::max_E(int& row, int& col, double& maxE) {
    maxE = fabs(a[0][1]);
    row = 0;
    col = 1;
    for (int i = 0; i < nDim; i++){
